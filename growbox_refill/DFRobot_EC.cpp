...
void DFRobot_EC::calibrationLCD(float voltage, float temperature, LiquidCrystal lcd)
{
    this->_voltage = voltage;
    this->_temperature = temperature;
    ecCalibrationLCD(lcd);  // if received Serial CMD from the serial monitor, enter into the calibration mode
}
...
void DFRobot_EC::ecCalibration(byte mode)
{
    ...
    switch(mode){
        ...
        if(enterCalibrationFlag){
            ...
            }else if((this->_rawEC>9)&&(this->_rawEC<30.8)){                    //recognize 12.88ms/cm buffer solution
            ...
            Serial.print("KValueTemp = RES2 * ECREF * compECsolution/1000/voltage\n");
            Serial.print("RES2 = ");
            Serial.println(RES2);
            Serial.print("ECREF = ");
            Serial.println(ECREF);
            Serial.print("compECsolution = ");
            Serial.println(compECsolution);
            Serial.print("voltage = ");
            Serial.println(this->_voltage);
            Serial.print("temp = ");
...
void DFRobot_EC::ecCalibrationLCD(LiquidCrystal lcd)
{
  //char *receivedBufferPtr;
  //static boolean ecCalibrationFinish  = 0;
  //static boolean enterCalibrationFlag = 0;
  ...
  lcd.clear();
  lcd.setCursor(0, 0);
  sprintf(tmp,"kVal: %.2f",(double) KValueTemp);
  lcd.print(tmp);
  delay(500);

  lcd.clear();
  lcd.setCursor(0, 0);
  sprintf(tmp, "RES2: %.2f", RES2);
  lcd.print(tmp);
  delay(500);

  lcd.clear();
  lcd.setCursor(0, 0);
  sprintf(tmp, "ECREF: %.2f", ECREF);
  lcd.print(tmp);
  delay(500);

  lcd.clear();
  lcd.setCursor(0, 0);
  sprintf(tmp, "ECsol: %.2f", (double) compECsolution);
  lcd.print(tmp);
  delay(500);

  lcd.clear();
  lcd.setCursor(0, 0);
  sprintf(tmp, "volt: %.2f",(double)  this->_voltage);
  lcd.print(tmp);
  delay(500);

  lcd.clear();
  lcd.setCursor(0, 0);
  sprintf(tmp, "temp: %.2f",(double)  this->_temperature);
  lcd.print(tmp);
  delay(500);
